...
const ArticleSearchContext = createContext<ArticleSearchContextValue | undefined>(undefined);

export const useArticleSearchContext = () => {
  const context = useContext(ArticleSearchContext);
  if (!context) {
    throw new Error("useArticleSearchContext must be used within an ArticleSearchContextProvider");
  }
  return context;
};
...
const ArticleListContext = createContext<ArticleListContextValue | undefined>(undefined);

export const useArticleListContext = () => {
  const context = useContext(ArticleListContext);
  if (!context) {
    throw new Error("useArticleListContext must be used within an ArticleListContextProvider");
  }
  return context;
};
...
export const ArticleListContextProvider: React.FC<ArticleListContextProps> = ({
  ...
  const { searchValue, filterValue, lunrResultSlugs } = articleSearchContextValue;

  const [sortValue, setSortValue] = useState<ArticleSortOption>(sortOptions[2]);
  const [currentPageIndex, setCurrentPageIndex] = useState<number>(originalPageIndex);
  const [articlesPerPage, setArticlesPerPage] = useState<number>(9);

  const currentSkipNumber = currentPageIndex * articlesPerPage;

  const sortedArticles = getSortedListViewArticles(articles, sortValue, lunrResultSlugs);
  ...
  if (!searchValue == "" || !filterValue.length == 0) {
    ...
    articlesToDisplay =getCurrentListViewPageArticles(sortedArticles, originalPageIndex, articlesPerPage);
  }
